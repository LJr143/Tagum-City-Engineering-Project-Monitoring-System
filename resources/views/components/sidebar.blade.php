    ...
    <div class="text-sm">
        <ul class="mt-10 space-y-1 tracking-wide">
            ...
            <li x-data="{toggle: false}" @mouseenter="toggle = true"
                @mouseleave="toggle = false"
                :style="toggle ? 'background-color: rgba(36, 144, 0, 0.3);' : ''">
                <a href="{{ route('manage-user') }}" @if (request()->routeIs('manage-user')){ @click.prevent="preventAction = true" }@endif aria-label="dashboard" @class(['relative flex items-center gap-3 px-6 py-0 text-Primary font-medium', ' text-customGreen font-semibold'=> request()->routeIs('manage-user')]) @mouseenter="toggle = true" @mouseleave="toggle = false"
                    @click="setActiveTab(''); show = false; showManage = false; subActiveItem = ''; setsubTmsActiveItem(''); setsettingsTmsActiveItem('')">
                    <div class=" text-center px-[13px] py-[10px] rounded-full justify-center items-center flex"
                        :class="{'bg-customGreen-100': {{ request()->routeIs('manage-user') ? 'true' : 'false' }}, 'bg-none': {{ !request()->routeIs('manage-user') ? 'true' : 'false' }}}">
                        <svg width="15" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <li x-data="{toggle: false}" @mouseenter="toggle = true"
                @mouseleave="toggle = false"
                :style="toggle ? 'background-color: rgba(36, 144, 0, 0.3);' : ''">
                <a href="{{ route('system-logs') }}" @if (request()->routeIs('system-logs')){ @click.prevent="preventAction = true" }@endif aria-label="dashboard" @class(['relative flex items-center gap-3 px-6 py-0 text-Primary font-medium', ' text-customGreen font-semibold'=> request()->routeIs('system-logs')]) @mouseenter="toggle = true" @mouseleave="toggle = false"
                   @click="setActiveTab(''); show = false; showManage = false; subActiveItem = ''; setsubTmsActiveItem(''); setsettingsTmsActiveItem('')">
                    <div class=" text-center px-[13px] py-[10px] rounded-full justify-center items-center flex"
                         :class="{'bg-customGreen-100': {{ request()->routeIs('system-logs') ? 'true' : 'false' }}, 'bg-none': {{ !request()->routeIs('system-logs') ? 'true' : 'false' }}}">
                        <svg width="15" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
