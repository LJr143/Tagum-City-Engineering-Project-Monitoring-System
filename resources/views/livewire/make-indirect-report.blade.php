<div x-data="{ makeIndirectReport: @entangle('makeIndirectReport') }" x-cloak @cost-added.window="makeIndirectReport = false">
    <div class="flex justify-end">
        <div class="relative ml-2">
            <button @click="makeIndirectReport = true"
    ...
    >
        ...
        >
            <div class="flex justify-between items-center mb-4 border-b border-gray-300 pb-2">
                ...
                <!-- Close Button (X) -->
                <button @click="makeIndirectReport = false" class="text-gray-500 hover:text-gray-700">
            ...
            <form wire:submit.prevent="saveMaterialUpdate" class="space-y-4 text-[12px]">
                ...
                <div class="w-full border border-black p-2 rounded-md">
                    ...
                    @foreach ($indirect_update as $index => $po)
                        <div class="flex gap-2 items-center mb-2">
                            <select name="indirect_update[{{ $index }}][indirect_cost_id]"
                                    wire:model.defer="indirect_update.{{ $index }}.indirect_cost_id"
                                    class="w-1/2 p-1 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 text-xs"
                                    required>
                                <option value="" disabled>Select Indirect Cost</option>
                                @foreach ($indirect_costs as $cost)
                                    <option value="{{ $cost->id }}">{{ $cost->description }}</option>
                                @endforeach
                            </select>

                            <!-- Input for Amount Used -->
                            <input type="number" name="indirect_update[{{ $index }}][amount_used]"
                                   wire:model.defer="indirect_update.{{ $index }}.amount_used"
                ...
                <div class="mt-6 flex justify-end space-x-2">
                    <button @click="makeIndirectReport = false" type="button"
    ...
    <script>
        document.addEventListener('indirect_update', () => {
            document.querySelector('[x-data]').__x.$data.makeIndirectReport = false;
        });
    </script>
</div>
