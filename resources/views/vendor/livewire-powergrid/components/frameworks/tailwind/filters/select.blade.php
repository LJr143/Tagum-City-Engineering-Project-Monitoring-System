...
@else
    ...
    >
        @if (!$inline)
            <label class="block text-[12px] font-semibold text-pg-primary-700 dark:text-pg-primary-300">
                {{ $title }}
            </label>
        @endif
        <div class="relative text-[12px]">
            <select
                class="{{ $filterClasses }} text-[12px]"
            style="{{ data_get($column, 'headerStyle') }}"
            {{ $defaultAttributes['selectAttributes'] }}
            >
            <option value="" class="text-[12px]">{{ trans('livewire-powergrid::datatable.select.all') }}</option>
